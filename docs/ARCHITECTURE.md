...
1. **`burn_records.id` 类型**：迁移 002 文档写 `bigint`，实际数据库是 `uuid`。`feed_likes.record_id` 和 `feed_comments.record_id` 必须与 `burn_records.id` 类型一致（uuid），否则外键创建失败。

2. **RLS 缺 UPDATE 策略**：迁移 002 只有 SELECT/INSERT/DELETE，没有 UPDATE。`publishToFeed` 用 PATCH 设 `is_public=true`，被 RLS 静默拦截。006 迁移补上 UPDATE + 公开 SELECT。

3. **`RETURNS TABLE` 列名冲突**：`get_public_feed` 返回 `record_id uuid`，PL/pgSQL 将其作为函数变量，与子查询 `feed_likes.record_id` 冲突 → `column reference "record_id" is ambiguous`。解决：子查询中加表前缀 `feed_likes.record_id`。
---
...
```
RLS：SELECT/INSERT/UPDATE/DELETE 仅限 `auth.uid() = user_id`，额外公开 SELECT（`is_public = true`）。

### feed_likes
```sql
id bigint GENERATED ALWAYS AS IDENTITY PRIMARY KEY
record_id uuid REFERENCES burn_records(id) ON DELETE CASCADE NOT NULL
user_id uuid REFERENCES auth.users(id) ON DELETE CASCADE NOT NULL
created_at timestamptz DEFAULT now()
UNIQUE(record_id, user_id)
```
RLS：本人管理（ALL）+ 所有人可查看（SELECT）。
...
- [x] burn_records.id uuid 类型修正 + RLS UPDATE 策略补全 + record_id 歧义修复
